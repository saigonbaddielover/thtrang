...
function Test-ContractProperties {
    ...
    if (-not (Test-ContractJsonObject $Value)) {
...
function Test-AllowedContractFields {
    ...
    if(-not(Test-ContractJsonObject $Value)){return}
...
function Test-ContextObjectArray {
    ...
    if(-not(Test-ContractJsonArray $Context.$Field)){Add-ContractIssue $Issues 'invalid-semantic-context' "diagram.context.$Field" 'Expected an array';return @()}
...
function Invoke-DrawioContractAudit {
    ...
    if($null-ne$manifest-and-not(Test-ContractJsonObject $manifest)){Add-ContractIssue $issues 'invalid-semantic-contract' 'semantic-manifest' 'Semantic manifest must be an object';$manifest=$null}
    if($null-ne$notation-and-not(Test-ContractJsonObject $notation)){Add-ContractIssue $issues 'invalid-notation-contract' 'notation-profile' 'Notation profile must be an object';$notation=$null}
    if($manifest){
        ...
        if($manifest.PSObject.Properties['schemaVersion']-and(-not(Test-ContractJsonInteger $manifest.schemaVersion)-or[int64]$manifest.schemaVersion-ne1)){Add-ContractIssue $issues 'semantic-schema-version' '' 'schemaVersion must be the integer 1';$manifestUsable=$false}
        foreach($arrayField in @('nodes','edges')){if($manifest.PSObject.Properties[$arrayField]-and-not(Test-ContractJsonArray $manifest.$arrayField)){Add-ContractIssue $issues 'invalid-semantic-contract' $arrayField "$arrayField must be an array";$manifestUsable=$false}}
        if($manifest.PSObject.Properties['ruleDispositions']-and-not(Test-ContractJsonArray $manifest.ruleDispositions)){Add-ContractIssue $issues 'invalid-semantic-contract' 'ruleDispositions' 'ruleDispositions must be an array';$manifestUsable=$false}
        if($manifest.PSObject.Properties['diagram']){
            ...
            if($manifest.diagram.PSObject.Properties['page']){$page=$manifest.diagram.page;if(-not(Test-ContractJsonObject $page)){Add-ContractIssue $issues 'invalid-semantic-contract' 'diagram.page' 'page must be an object';$manifestUsable=$false}else{Test-AllowedContractFields $page @('width','height') $issues 'diagram.page';foreach($field in @('width','height')){if(-not$page.PSObject.Properties[$field]){Add-ContractIssue $issues 'invalid-semantic-contract' "diagram.page.$field" 'Required field is missing'}else{Test-PositiveContractField $page $field $issues 'diagram.page'}}}}
            if($manifest.diagram.PSObject.Properties['standard']){$standardValue=$manifest.diagram.standard;if(-not(Test-ContractJsonObject $standardValue)){Add-ContractIssue $issues 'invalid-semantic-contract' 'diagram.standard' 'standard must be an object';$manifestUsable=$false}else{Test-AllowedContractFields $standardValue @('name','version','source') $issues 'diagram.standard';foreach($field in @('name','source')){if(-not$standardValue.PSObject.Properties[$field]){Add-ContractIssue $issues 'invalid-semantic-contract' "diagram.standard.$field" 'Required field is missing'}else{Test-NonemptyContractField $standardValue $field $issues 'diagram.standard'}}}}
        }
    ...
    if($notation){
        ...
        if($notation.PSObject.Properties['schemaVersion']-and(-not(Test-ContractJsonInteger $notation.schemaVersion)-or[int64]$notation.schemaVersion-ne1)){Add-ContractIssue $issues 'notation-schema-version' '' 'schemaVersion must be the integer 1';$notationUsable=$false}
        foreach($objectField in @('provenance','nodeTypes','edgeTypes')){if($notation.PSObject.Properties[$objectField]-and-not(Test-ContractJsonObject $notation.$objectField)){Add-ContractIssue $issues 'invalid-notation-contract' $objectField "$objectField must be an object";$notationUsable=$false}}
        if($notation.PSObject.Properties['rules']-and-not(Test-ContractJsonArray $notation.rules)){Add-ContractIssue $issues 'invalid-notation-contract' 'rules' 'rules must be an array';$notationUsable=$false}
        ...
        if($notation.PSObject.Properties['nodeTypes']-and(Test-ContractJsonObject $notation.nodeTypes)){foreach($entry in @($notation.nodeTypes.PSObject.Properties)){Test-AllowedContractFields $entry.Value @('styleMatchers','shapeClass','container') $issues "nodeTypes.$($entry.Name)";$notationUsable=(Test-ContractProperties $entry.Value @('shapeClass','styleMatchers') $issues 'invalid-notation-node-type' $entry.Name)-and$notationUsable;if($entry.Value.PSObject.Properties['styleMatchers']){if(-not(Test-ContractJsonArray $entry.Value.styleMatchers)){Add-ContractIssue $issues 'invalid-notation-node-type' $entry.Name 'styleMatchers must be an array';$notationUsable=$false}else{$matchers=@($entry.Value.styleMatchers);if($matchers.Count-eq0){Add-ContractIssue $issues 'invalid-notation-node-type' $entry.Name 'styleMatchers must contain at least one entry';$notationUsable=$false};foreach($matcher in $matchers){Test-AllowedContractFields $matcher @('allOf','noneOf') $issues "nodeTypes.$($entry.Name).styleMatchers";if(-not$matcher.PSObject.Properties['allOf']-or-not(Test-ContractJsonArray $matcher.allOf)-or@($matcher.allOf).Count-eq0){Add-ContractIssue $issues 'invalid-notation-node-type' $entry.Name 'Matcher allOf must be a nonempty array';$notationUsable=$false};if($matcher.PSObject.Properties['noneOf']-and-not(Test-ContractJsonArray $matcher.noneOf)){Add-ContractIssue $issues 'invalid-notation-node-type' $entry.Name 'Matcher noneOf must be an array';$notationUsable=$false}}}}}}
        ...
        if($notation.PSObject.Properties['rules']-and(Test-ContractJsonArray $notation.rules)){foreach($rule in @($notation.rules)){Test-AllowedContractFields $rule @('id','severity','description') $issues 'rules';foreach($field in @('id','severity','description')){if(-not$rule.PSObject.Properties[$field]){Add-ContractIssue $issues 'invalid-notation-rule' ([string]$rule.id) "$field is required";$notationUsable=$false}};Test-EnumContractField $rule 'severity' @('ERROR','WARNING','INFO') $issues 'rules'}}
    }
    if($manifestUsable){
