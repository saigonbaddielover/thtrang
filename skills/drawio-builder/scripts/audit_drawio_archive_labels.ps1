...
function Add-Issue {
    ...
    $repairClass = if ($Type -match 'clearance|label') { 'resize-or-reposition-label' } else { 'rerender' }
    Add-DrawioIssue -Issues $Issues -Type $Type -Element $Shape -Detail $Detail -Evidence $Detail -RepairClass $repairClass
}
...
function Get-PathPoints {
    ...
    if ($points.Count -gt 1 -and [math]::Abs($points[0].X-$points[$points.Count-1].X)-le$Epsilon -and [math]::Abs($points[0].Y-$points[$points.Count-1].Y)-le$Epsilon) { return @($points[0..($points.Count-2)]) }
    $points
}
...
foreach ($cell in $archiveCells) {
    ...
    try {
        ...
        $ink = Get-SvgLabelInkBounds -Group $group -Namespace $namespace
        if (-not $ink) {
...
$result = [pscustomobject]@{
    ...
    ErrorCount = @($issues | Sort-Object Severity, Type, Element, Detail -Unique | Where-Object Severity -eq 'ERROR').Count
    WarningCount = @($issues | Sort-Object Severity, Type, Element, Detail -Unique | Where-Object Severity -eq 'WARNING').Count
    IssueCount = @($issues | Sort-Object Severity, Type, Element, Detail -Unique).Count
    Issues = @($issues | Sort-Object Severity, Type, Element, Detail -Unique)
}
