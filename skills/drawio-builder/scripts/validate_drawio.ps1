...
$inputDigests = Get-DrawioInputDigests -SourcePath $SourcePath -SvgPath $SvgPath -QualityProfilePath $QualityProfilePath -SemanticManifestPath $SemanticManifestPath -NotationProfilePath $NotationProfilePath -ArtifactManifestPath $ArtifactManifestPath
        ...
        if($visual){
            ...
            elseif(-not(Test-Path -LiteralPath $CropReportPath -PathType Leaf)){Add-DrawioIssue -Issues $visualIssues -Type 'visual-inspection-crop-report' -Element '' -Detail 'CropReportPath does not exist' -Evidence $CropReportPath -RepairClass 'repeat-visual-inspection'}
                ...
                if($cropReport){
                    ...
                    if($cropReport.PSObject.Properties['Png']){$declaredPng=[string]$cropReport.Png;if(-not[System.IO.Path]::IsPathRooted($declaredPng)){$declaredPng=Join-Path $reportDirectory $declaredPng};if(-not(Test-Path -LiteralPath $declaredPng -PathType Leaf)){Add-DrawioIssue -Issues $visualIssues -Type 'visual-inspection-crop-report-binding' -Element 'Png' -Detail 'Crop report source PNG does not exist' -Evidence ([string]$cropReport.Png) -RepairClass 'repeat-crop-export'}elseif($ArtifactManifestPath){try{$artifactManifest=Get-Content -LiteralPath $ArtifactManifestPath -Raw -Encoding UTF8|ConvertFrom-Json;$artifactBase=Split-Path -Parent (Resolve-Path -LiteralPath $ArtifactManifestPath).Path;$pngArtifacts=@($artifactManifest.artifacts|Where-Object{[string]$_.role-eq'png'});if($pngArtifacts.Count-ne1){Add-DrawioIssue -Issues $visualIssues -Type 'visual-inspection-artifact-binding' -Element 'Png' -Detail 'Artifact manifest must contain exactly one PNG' -Evidence ([string]$pngArtifacts.Count) -RepairClass 'regenerate-artifacts'}else{$artifactPng=Join-Path $artifactBase ([string]$pngArtifacts[0].path);if(-not(Test-Path -LiteralPath $artifactPng -PathType Leaf)-or(Resolve-Path -LiteralPath $artifactPng).Path-ne(Resolve-Path -LiteralPath $declaredPng).Path){Add-DrawioIssue -Issues $visualIssues -Type 'visual-inspection-artifact-binding' -Element 'Png' -Detail 'Crop report PNG differs from artifact manifest PNG' -Evidence "$declaredPng != $artifactPng" -RepairClass 'regenerate-artifacts'}}}catch{Add-DrawioIssue -Issues $visualIssues -Type 'visual-inspection-artifact-binding' -Element 'Png' -Detail $_.Exception.Message -Evidence $ArtifactManifestPath -RepairClass 'regenerate-artifacts'}}}
    ...
    if(-not$WarningDispositionPath){Add-StateGate 'warning-dispositions' 'UNKNOWN' 'Warnings were emitted and WarningDispositionPath was not supplied' 'Every warning requires an accepted or fixed disposition before approval'}
        ...
        Invoke-Gate 'warning-dispositions' {
            ...
            foreach($warning in $warningRecords){
                ...
                if($match.Count-ne1-or$match[0].decision-notin@('accepted','fixed')-or[string]::IsNullOrWhiteSpace([string]$match[0].reason)){Add-DrawioIssue -Issues $dispositionIssues -Type 'missing-warning-disposition' -Element $warning.Element -Detail "$($warning.Gate)/$($warning.Type)" -Evidence "$($warning.Gate)/$($warning.Type)/$($warning.Element)" -RepairClass 'disposition-warning'}
            }
