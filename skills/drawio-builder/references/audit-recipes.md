...
For a warning-bearing page, bootstrap in this order: run Approval once to discover warnings, write dispositions, rerun Approval without visual evidence to create the authoritative pre-visual report, export crops from that report, inspect the full page and crops, then run final Approval with the same dispositions and contract inputs. Never export final crops from the discovery report. For each `EXPORTED` crop, its review ID is the filename stem of `Crops[].Path`; record that exact value and the crop file's lowercase SHA-256 in `reviewedCrops`.
